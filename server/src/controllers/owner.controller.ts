...
export const addVehicle = async (req: Request, res: Response): Promise<void> => {
  try {
    const { ownerId, chassisNo, engineNo, regdNo, fuelType, insuranceCompany, insurancePolicyNo, insuranceValidUpto, puccNo, puccValidUpto, insuranceDocument,
      pucDocument } = req.body;
...
export const manageVehicle = async (req: Request, res: Response) => {
  try {
    const { vehicleId } = req.params;
...
export const getDriversByOwner = async (req: Request, res: Response): Promise<void> => {
  try {
    const { ownerId } = req.params; // Assuming ownerId is passed as a route parameter
...
export const removeDriverById = async (req: Request, res: Response): Promise<void> => {
  try {
...
export const getFreeVehiclesByOwner = async (req: Request, res: Response): Promise<void> => {
  try {
  ...
  try {
    ...
    if (role === 'owner') {
      notifications = await prisma.notificationLog.findMany({
        where: { ownerId: parseInt(userId as string, 10) },
        include: {
          vehicle: true,
          driver: true,
        },
      ...
      notifications = await prisma.notificationLog.findMany({
        where: { driverId: parseInt(userId as string, 10) },
        include: {
          vehicle: true,
          owner: true,
        },
