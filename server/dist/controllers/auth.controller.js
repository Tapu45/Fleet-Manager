...
exports.driverSignup = exports.ownerSignup = exports.login = void 0;
...
const login = async (req, res) => {
    try {
        ...
        if (owner) {
            ...
            const isPasswordValid = await bcryptjs_1.default.compare(password, owner.password);
            if (!isPasswordValid) {
                return res.status(401).json({ error: 'Invalid credentials' });
            }
            // Generate a JWT token
            const token = jsonwebtoken_1.default.sign({ id: owner.id, email: owner.email, role: 'owner' }, process.env.JWT_SECRET || 'your_jwt_secret', { expiresIn: '1d' });
            // Trigger vehicle alerts after successful login
            await (0, alert_1.triggerVehicleAlerts)();
        ...
        if (driver) {
            // Compare the password
            const isPasswordValid = await bcryptjs_1.default.compare(password, driver.password);
            if (!isPasswordValid) {
                return res.status(401).json({ error: 'Invalid credentials' });
            }
            // Generate a JWT token
            const token = jsonwebtoken_1.default.sign({ id: driver.id, email: driver.email, role: 'driver' }, process.env.JWT_SECRET || 'your_jwt_secret', { expiresIn: '1d' });
    ...
    try {
        ...
        if (!paymentId) {
            ...
            const order = await razorpay.orders.create({
                ...
                currency: 'INR',
                receipt: crypto_1.default.randomBytes(10).toString('hex'), // Generate a random receipt ID
            });
